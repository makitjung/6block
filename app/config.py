...
# 오늘 '달성'을 종일 이벤트로 쓰는 '성과' 캘린더. 같은 서비스계정으로 쓰며,
# 그 캘린더를 서비스계정 이메일에 '변경 권한'으로 공유해야 동작한다. 비우면 성과 쓰기만 비활성.
GCAL_WRITE_ACHIEVE_CALENDAR_ID = os.getenv("GCAL_WRITE_ACHIEVE_CALENDAR_ID", "").strip()
...
DEFAULT_SETTINGS = {
    ...
    "show_reflect": "0",        # 슬롯 '고민' 버튼 표시(0/1)
    "show_slot_play": "0",      # 슬롯 '▶'(포모도로 시작) 버튼 표시(0/1)
    "show_inbox": "0",          # 오늘 탭 빠른 수집함 표시(0/1). 주간 탭에는 수집함이 없다.
    "weekday_concepts": "",     # 요일별 컨셉 7칸(JSON 배열, 0=월 ~ 6=일)
    # 화면에 안 보이게 할 Things3 할일 제목(쉼표로 여러 개). 매일 뜨는 점검용 항목처럼
    # 목록에 늘 있지만 볼 필요가 없는 것을 걸러 낸다. 제목이 정확히 같은 것만 걸린다.
    "hide_task_titles": "체크/잡일(2분)",
