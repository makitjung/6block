...
- **색은 영역, 진하기는 계층 단계입니다** · 영역(프로젝트·투자·학습…)마다 색이 하나 있고, 그 색 안에서 **최상위가 가장 진하고 하위로 내려갈수록 옅어집니다**(4단계). 영역 색은 `영역 관리`에서 팔레트 8종 중에 고릅니다.
...
- **막대 끌어 옮기기**: **좌우로 끌면 주 단위(7일)로 붙어** 기간이 옮겨지고 길이는 그대로입니다. 끄는 동안 막대가 **놓일 자리로 한 주씩 건너뛰며** 움직이고 `+2주 · 7/20~8/16` 처럼 바뀔 기간이 떠서, 보이는 자리가 그대로 저장됩니다(한 주가 안 되게 끌면 막대가 아예 안 움직여 바로 알 수 있습니다). 하위가 있으면 하위 사슬도 같은 날수만큼 함께 밀립니다. **양 끝 16px 안(마우스를 올리면 세로 손잡이가 보입니다)을 잡아 끌면** 그쪽 끝만 주 단위로 늘거나 줄어듭니다. **정확한 날짜는 막대를 눌러 편집창에 적습니다**(하루 단위로 그대로 저장됩니다).
