...
- **연결선 두 가지** · 상위와 하위는 **옅은 실선 갈고리**로 이어져 어느 계획에 딸린 것인지 보입니다. 한 항목이 여러 블록에 걸려 나뉜 것은 **희미한 점선 곡선**으로 이어집니다(최상위 항목만 긋습니다. 하위까지 다 그으면 화면이 복잡해집니다).
...
- **막대 끌어 옮기기**: **좌우로 끌면 주 단위(7일)로 붙어** 기간이 옮겨지고 길이는 그대로입니다. 하위가 있으면 하위 사슬도 같은 날수만큼 함께 밀립니다. **양 끝을 끌면** 그쪽 끝만 주 단위로 늘거나 줄어듭니다. **정확한 날짜는 막대를 눌러 편집창에 적습니다**(하루 단위로 그대로 저장됩니다).
