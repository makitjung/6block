...
- **막대 끌어 옮기기**: **좌우로 끌면 주 단위(7일)로 붙어** 기간이 옮겨지고 길이는 그대로입니다. 하위가 있으면 하위 사슬도 같은 날수만큼 함께 밀립니다. **양 끝 16px 안(마우스를 올리면 세로 손잡이가 보입니다)을 잡아 끌면** 그쪽 끝만 주 단위로 늘거나 줄어듭니다. **정확한 날짜는 막대를 눌러 편집창에 적습니다**(하루 단위로 그대로 저장됩니다).
