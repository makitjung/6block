...
- **왼쪽 줄은 코어블록 `B1`~`B6`**(+ 블록을 안 정한 항목이 모이는 `미지정` 한 줄)이고 **위쪽은 기간 열**입니다. 항목마다 블록 하나를 정하고, 안 정하면 상위 항목의 블록을 따라갑니다.
- **색은 영역, 진하기는 기간입니다** · 영역(프로젝트·투자·학습…)마다 색이 하나 있고, 그 색 안에서 장기(6개월↑)가 가장 진하고 중기(1~6개월) · 단기(1개월↓)를 거쳐 초단기(1주↓)가 가장 옅습니다. 짧은 쪽을 먼저 봐서 7일 이하는 초단기입니다. 영역 색은 `영역 관리`에서 팔레트 8종 중에 고릅니다.
- 같은 블록 줄 안에서 **하위 기간 막대가 상위 막대 안에 겹쳐** 그려집니다(연 안에 분기, 분기 안에 월). 하위에 블록을 따로 주면 그 블록 줄로 빠집니다.
- 막대를 누르면 이름 · 블록 · 영역 · 기간 · 진척률 편집과 `하위 추가`가 열리고, 막대 오른쪽 끝 `✕`로 바로 지웁니다. 하위가 있는 항목의 기간·진척률은 하위에서 자동 계산됩니다. 편집창에서 **영역을 바꾸면** 하위 항목은 상위에서 빠져 그 영역의 최상위가 됩니다.
- **막대 끌어 옮기기**: 가운데를 좌우로 끌면 보고 있는 열 단위(연·분기·월·주)로 기간이 옮겨지고 길이는 그대로입니다. **양 끝을 끌면** 그쪽 끝만 움직여 기간이 늘거나 줄어듭니다. **위아래 다른 블록 줄에 놓으면** 그 블록으로 옮겨지고, 다른 막대 위에 놓으면 그 막대의 **하위계획**이 되어 안쪽에 겹쳐 그려집니다.
