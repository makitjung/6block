...
- **한 줄 안에서 막대는 서로 가리지 않습니다.** 기간이 겹치면 아래 칸으로 내려 담고, 블록마다 적어도 두 칸을 두어 한 블록에 두 항목이 나란히 보입니다. 상하위 관계는 자리가 아니라 색 진하기로만 나타냅니다. 하위에 블록을 따로 주면 그 블록 줄로 빠집니다.
...
- **적어 넣은 기간은 하위가 있어도 그대로 남습니다.** 하위가 상위 밖으로 삐져나오면 그대로 보입니다(예전에는 하위를 품도록 되돌려 놔서 상위 항목은 날짜를 고쳐도 안 바뀌는 것처럼 보였습니다).
- **막대 끌어 옮기기**: **좌우로 끌면 끈 만큼(하루 단위)** 기간이 옮겨지고 길이는 그대로입니다. 하위가 있으면 하위 사슬도 같은 날수만큼 함께 밀립니다. **양 끝을 끌면** 그쪽 끝만 움직여 기간이 늘거나 줄어듭니다.
- **하위계획으로 넣기**: 끌고 가서 **다른 막대 위에서 잠깐(0.35초) 멈추면** 양쪽에 표시가 켜지고, 그대로 놓으면 그 막대의 하위가 됩니다. 같은 줄이든 다른 줄이든 됩니다. 계속 움직이는 동안에는 잡히지 않으니 옆으로 미는 도중 엉뚱하게 붙지 않습니다. 다른 블록 줄 **빈 자리**에 놓으면 그 블록으로 옮겨집니다. 편집창의 `상위 항목`으로 골라도 됩니다.
