...
- **한 항목을 여러 블록에 걸 수 있습니다.** 편집창에서 `B2` `B6`처럼 여러 개를 고르면 같은 막대가 그 줄마다 나오고, 어느 줄에서 끌어도 같이 움직입니다. 하나의 프로젝트를 두 블록에서 동시에 돌릴 때 씁니다. 그렇게 나뉜 막대들은 **옅은 세로선으로 이어져** 원래 하나라는 게 보입니다.
...
- **옮긴 막대는 사라지지 않습니다.** 끌어서 과거로 보내거나 보이는 기간 밖으로 보내면, 그 항목이 보이는 자리로 화면이 따라가고 접힘도 자동으로 풀린 뒤 잠깐 깜빡여 알려 줍니다.
