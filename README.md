...
- **오늘 자리에 빨간 세로선**이 그어지고 머리글에 `오늘` 깃발이 붙습니다. 진척률은 막대 안쪽을 같은 색 더 진하게 채워 보여 주고, 오른쪽 끝에 숫자로도 적습니다.
...
- 막대를 누르면 편집칸이 **두 줄**로 열립니다(첫 줄 이름·진척, 둘째 줄 기간·블록·영역). `저장`을 누르거나 **격자의 빈 곳을 누르면 닫힙니다.** 지우기는 막대 오른쪽 끝 `✕`, 상위로 넣기는 끌어다 붙이기로 합니다. 하위가 있는 항목의 진척률은 하위 평균입니다. 편집창에서 **영역을 바꾸면** 하위 항목은 상위에서 빠져 그 영역의 최상위가 됩니다.
