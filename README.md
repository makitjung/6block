...
- **막대 끌어 옮기기**: 좌우로 끌면 보고 있는 열 단위(연·분기·월·주)로 기간이 옮겨지고 길이는 그대로입니다. 다른 막대 위에 놓으면 그 막대의 **하위계획**이 되어 안쪽에 겹쳐 그려지고, 영역 줄에 놓으면 다시 최상위로 빠집니다.
...
임시 폴더에 새 DB를 만들고 127.0.0.1:8011에 테스트 서버를 띄워 실제 HTTP 요청으로 71개 항목을
...
하루 골격 생성의 멱등성, 필드 자동저장, 블록 시간 변경 시 장소 보존, 장기 계획 막대(추가·하위·자동 롤업·겹침·기간 이동·하위지정·삭제),
