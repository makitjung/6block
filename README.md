...
- **이번 주 장기 항목**: 그 주에 걸친 장기 계획 막대를 모아 진척률을 바로 고치고, `주간 목표로`·`블록으로` 버튼으로 옮깁니다.
- 주간에서 정한 블록 이름이 일간 기본값이 되고, 일간에서 덮어쓸 수 있음.

### 장기 화면 (`/plan`)

- 연 · 분기 · 월 · 주 단위로 보는 **계획 막대** 하나로 관리합니다(표 없음).
- 최상위 항목이 한 줄이고 **하위 기간 막대가 그 안에 겹쳐** 그려집니다(연 안에 분기, 분기 안에 월).
- **기간 길이로 색이 갈립니다** · 장기(6개월↑) 파랑 · 중기(1~6개월) 청록 · 단기(1개월↓) 주황 · 초단기(1주↓) 보라. 짧은 쪽을 먼저 봐서 7일 이하는 초단기입니다.
- 막대를 누르면 이름 · 기간 · 진척률 편집과 `하위 추가`가 열리고, 막대 오른쪽 끝 `✕`로 바로 지웁니다. 하위가 있는 항목의 기간·진척률은 하위에서 자동 계산됩니다.
- **막대 끌어 옮기기**: 가운데를 끌면 보고 있는 열 단위(연·분기·월·주)로 기간이 옮겨지고 길이는 그대로입니다. **양 끝을 끌면** 그쪽 끝만 움직여 기간이 늘거나 줄어듭니다. 다른 막대 위에 놓으면 그 막대의 **하위계획**이 되어 안쪽에 겹쳐 그려지고, 영역 줄에 놓으면 다시 최상위로 빠집니다.
...
하루 골격 생성의 멱등성, 필드 자동저장, 블록 시간 변경 시 장소 보존, 장기 계획 막대(추가·하위·자동 롤업·겹침·기간 이동·기간 조절·기간 4구분·하위지정·삭제),
